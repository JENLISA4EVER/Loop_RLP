...
class DataCollatorWithPadding:
    ...
    def __call2__(self, batch):
        # 1. get prompts and responses
        if self.conversation_key in batch[0]:
            prompts = []
            responses = []
            for item in batch:
                conversations = item[self.conversation_key]
                for conversation in conversations:
                    if conversation[self.role_key] == self.user_key:
                        prompts.append(conversation[self.content_key])
                    if conversation[self.role_key] == self.assistant_key:
                        responses.append(conversation[self.content_key])
        else:
            prompts = [item[self.prompt_key] for item in batch]
            responses = [item[self.response_key] for item in batch]

        # 2. apply chat template
        if self.apply_chat:
            prompts = [self.tokenizer.apply_chat_template(prompt, add_generation_prompt=True, tokenize=False) for prompt in prompts]
        else:
            prompts = [f"Problem: {prompt}\nSolution: " for prompt in prompts]
        responses = [response + self.tokenizer.eos_token for response in responses] #FIXME:ouro的tokenizer.eos_token和config.eos_token不一样
        ...
        response_attention_mask = response_ids_output["attention_mask"][0]
        input_ids = [torch.cat((prompt_id, response_id), dim=-1) for prompt_id, response_id in zip(prompt_ids, response_ids)]
        attention_mask = [torch.cat((prompt_attention_mask, response_attention_mask), dim=-1) for prompt_attention_mask, response_attention_mask in zip(prompt_attention_mask, response_attention_mask)]
        if "label" in batch:
