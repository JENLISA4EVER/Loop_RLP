...
from utils.dataset_pt import OpenThoughtsDataset,DataCollatorWithPadding,OmniMathNTPDataset,OmniMathDataset
...
def main(config: DictConfig):
    ...
    if os.environ.get("RANK","-1")=="0" and os.environ.get("DEBUG_MODE","0")=="1":
    ...
    if CONFIG["dataset_name"] == "OpenThoughts":
    ...
    elif CONFIG["dataset_name"] == "OmniMathNTP":
        if os.path.isdir(CONFIG["train_data_path"]):    
            train_data_files = glob.glob(os.path.join(CONFIG["train_data_path"], "*.jsonl"))
        else:
            train_data_files = [CONFIG["train_data_path"]]
        if os.path.isdir(CONFIG["val_data_path"]):
            val_data_files = glob.glob(os.path.join(CONFIG["val_data_path"], "*.jsonl"))
        else:
            val_data_files = [CONFIG["val_data_path"]]
        train_dataset = OmniMathNTPDataset(train_data_files, tokenizer, max_length=CONFIG["max_length"],prompt_key=CONFIG["prompt_key"],response_key=CONFIG["response_key"])
        val_dataset = OmniMathDataset(val_data_files, tokenizer,max_length=CONFIG["max_length"],truncation="no")
    elif CONFIG["dataset_name"] == "OmniMathDataset":
        if os.path.isdir(CONFIG["train_data_path"]):
            train_data_files = glob.glob(os.path.join(CONFIG["train_data_path"], "*.jsonl"))
        else:
            train_data_files = [CONFIG["train_data_path"]]
        if os.path.isdir(CONFIG["val_data_path"]):
            ...
            val_data_files = [CONFIG["val_data_path"]]
        train_dataset = OmniMathDataset(train_data_files, tokenizer, max_length=CONFIG["max_length"],truncation="no")
        val_dataset = OmniMathDataset(val_data_files, tokenizer, max_length=CONFIG["max_length"],truncation="no")
    collate_fn = DataCollatorWithPadding(tokenizer, max_length=CONFIG["max_length"], truncation=CONFIG["truncation"], max_length_method=CONFIG.get("max_length_method", "fix"))
    train_dataloader = StatefulDataLoader(
        train_dataset, batch_size=CONFIG["batch_size"], shuffle=True, num_workers=CONFIG.get("num_workers",4),drop_last=True, collate_fn=collate_fn
    )
    val_dataloader = StatefulDataLoader(
        val_dataset, batch_size=CONFIG["val_batch_size"], shuffle=False, num_workers=CONFIG.get("num_workers",4),drop_last=False, collate_fn=collate_fn
    )
    ...
    accelerator.register_for_checkpointing(trainer.lr_scheduler)
    #--- Specai Setting --- #
    last_global_step = 0
    last_step = 0
    if CONFIG.get("resume_from_checkpoint", False):
        last_global_step = trainer.load_checkpoint()
        last_step = last_global_step * CONFIG["gradient_accumulation_steps"] * accelerator.num_processes
        if accelerator.is_main_process:
            logger.info(f"Resuming from global step {last_global_step}")
    # --- Training Loop ---
    global_step = last_global_step
    step = 0
    gradient_accumulation_step_idx = 0
    total_steps = (len(trainer.train_dataloader) * CONFIG["num_epochs"]) // (CONFIG["gradient_accumulation_steps"]*accelerator.num_processes)
    if accelerator.is_main_process:
        bar = tqdm(total=len(trainer.train_dataloader)*CONFIG["num_epochs"]//CONFIG["gradient_accumulation_steps"], desc="Training")
    for epoch in range(CONFIG["num_epochs"]):
        for _, batch in enumerate(trainer.train_dataloader):
            step += 1
            if step <= last_step:
                if accelerator.is_main_process:
                    logger.info(f"Skipping step {step} because it is less than last step {last_step}")
                continue
            metrics = {}
            with accelerator.accumulate(trainer.student):
                if os.environ.get("RANK","-1")=="0" and "3" in os.environ.get("DEBUG_MODE", "0"):
                    breakpoint()
                try:
                    ...
                    logger.error(f"RANK: {os.environ.get('RANK','-1')} Error in compute_loss: {e}")
                    from traceback import format_exc
                    logger.error(format_exc())
                    raise e
                accelerator.backward(loss)
                append_to_dict(metrics, train_metrics)
                gradient_accumulation_step_idx += 1
                if accelerator.is_main_process:
                    logger.info(f"Gradient accumulation step {gradient_accumulation_step_idx} of {CONFIG['gradient_accumulation_steps']} in global step {global_step}")
                if accelerator.sync_gradients:
                    ...
                    if global_step % CONFIG["log_interval"] == 0:
                        if accelerator.is_main_process:
                            ...
                            accelerator.log({"train/lr":lr_scheduler.get_last_lr()[0]},global_step)
            ...
            if (
                ...
                and global_step > 0 and gradient_accumulation_step_idx == 0
            ) :
                if accelerator.is_main_process:
                ...
                if accelerator.is_main_process:#FIXME:pred_num
                ...
                accelerator.wait_for_everyone()
            # if ((global_step) % CONFIG["save_interval"] == 0 and global_step > 0 and gradient_accumulation_step_idx == 0) or global_step==total_steps:
            #     trainer._save_checkpoint(global_step=global_step)
            #     accelerator.wait_for_everyone()
        if accelerator.is_main_process:
