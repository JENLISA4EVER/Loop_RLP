...
def prepare_rpt_dataset(base_model, tokenizer,CONFIG):
    """
        ...
        for i, line in enumerate(lines):
            ...
            try:
                ...
                for idx in valid_indices:
                    ...
                    category = None 
                    for cat, threshold in sorted(CONFIG["entropy_thresholds"].items(), key=lambda x: x[1]):#利用sorted函数对阈值进行排序，从低到高
                        if threshold <= h :
                            category = cat
                    
                    if category is not None and len(categorized_samples[category]) < CONFIG["samples_per_category"]:
...
def main(config: DictConfig):
    ...
    categorized_data = prepare_rpt_dataset(base_model, tokenizer,CONFIG)
    ...
    for cat, samples in categorized_data.items():
        ...
        acc_peak, _ = evaluate_samples(base_model, samples, mode="peak",CONFIG=CONFIG)
        acc_adapt, steps_adapt = evaluate_samples(base_model, samples, mode="adaptive",CONFIG=CONFIG)
