...
@dataclass
class HiddenStates():
    # https://github.com/huggingface/accelerate/issues/2638
    def __init__(self,hidden_states_list: Optional[tuple[torch.Tensor]]):
...
class OuroRLPOutput(CausalLMOutputWithPast):
    ...
    hidden_states_container: HiddenStates=None
...
class OuroForCausalLM(OuroPreTrainedModel, GenerationMixin):
    ...
    def forward(
        ...
        no_need_keys: Optional[list[str]] = None,
        **kwargs: Unpack[TransformersKwargs],
    ) -> CausalLMOutputWithPast:
        r"""
        Args:
        ...
        if not kwargs.get("disable_cast",False):
            if no_need_keys is not None:
                no_need_return_keys = no_need_keys #只限定于hidden_states_list,gate_list,logits
                if "logits" in no_need_return_keys:
                    logits = None
                if "hidden_states_list" in no_need_return_keys:
                    hidden_states_list = None
                if "gate_list" in no_need_return_keys:
                    gate_list = None
                if "add_noise_list" in no_need_return_keys:
                    add_noise_list = None
            ...
            result = OuroRLPOutput(
                ...
                attentions=outputs.attentions,
                # 【新增】直接在构造函数里传入，这是合法的
                hidden_states_container=HiddenStates(hidden_states_list=(
                    tuple(hidden_states_list) if hidden_states_list else None
                )),
                gate_list=tuple(gate_list) if gate_list else None,
                add_noise_list=tuple(add_noise_list) if add_noise_list else None,
